...
public final class Tunables {
  ...
  public static interface Tunable {
    /**
     * Initializes the object to be tuned. This method is called when adding the object to a {@link
     * TunableTable}.
     *
  ...
  /**
   * Interface for wrapping objects to be tunable. Instances of this interface should be passed to
   * {@link Tunables#registerHandler(Class, TunableHandler)} in order for objects of the handler's
  ...
  /**
   * Polls changes from NetworkTables. Must be called periodically in order for Tunables to function
   * (this is already done if utilizing {@link LoggedRobot}).
   */
  ...
  /**
   * Adds an object to be tuned. The specified object must implement {@link Tunable}, or have a
   * registered {@link TunableHandler} in order for it to be added.
   *
   * @param <T> The object's class.
  ...
  public static final class TunableBoolean implements TunableValue {
    ...
    /**
     * Adds a listener that will be invoked when the tunable's value is modified. Will no-op if the
